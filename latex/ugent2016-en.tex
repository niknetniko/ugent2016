...
\end{document}
    ...
    \begin{table}[h]
        \begin{center}
            ...
            \begin{tabular}{l l l}
                ...
                \texttt{layout} & \texttt{all} &  See Section~\ref{subsubsec:layout} \\
    ...
    This option specifies to which degree the document should be styled. Each level also applies the style of the previous level. For example, level 3 will also apply the style of level 2. The hierarchy looks like: \texttt{none} → \texttt{margins} → \texttt{colours} → \texttt{titlestyle} → \texttt{titlefont} → \texttt{all}.
    ...
    \begin{enumerate}
        ...
        \item \textbf{\texttt{all}} -- Apply everything. This means all fonts will be adjusted. Since there are no italics in the University's font, Arial will be used. This is the default value.
    \end{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{\texttt{campus}} -- See Section~\ref{subsubsec:campus}. The default value is \texttt{default}.
    \end{itemize}
...
\includegraphics{\ugentdefault}
    ...
    As mentioned in Section~\ref{subsec:taal}, the images are automatically chosen based on the document's language. There are experimental commands\footnote{Experimental means they can change or disappear between releases} that always give the same images. To use them, add the two letter language code to the commands, for example:

    \begin{minted}{latex}
\ugentdefault       % depends on language
\ugentdefaultnl     % always Dutch
\ugentdefaulten     % always English
    \end{minted}

    Following commands are available:

    \begin{description}[style=nextline]
        \item[\Q{\\ugentdefault}] Provides the default logo. This logo (and other logos) contains the minimal amount of whitespace.
            \begin{marginfigure}[-1cm]\includegraphics[width=\marginparwidth]{\ugentdefault}\end{marginfigure}
        \item[\Q{\\ugentkortrijk}] Logo of Campus Kortrijk. No translation is available.
            \begin{marginfigure}[-0.5cm]\includegraphics[width=\marginparwidth]{\ugentkortrijk}\end{marginfigure}
        \item[\Q{\\ugentglobal}] Logo of the Global Campus. No translation is available.
            \begin{marginfigure}[-0.5cm]\includegraphics[width=\marginparwidth]{\ugentglobal}\end{marginfigure}
        \item[\Q{\\ugentfacultylogo{code}}] Logo of the faculty. De parameter \texttt{code} is the two letter code from Table~\ref{table:faculty}. Note that the special value \texttt{none} cannot be used in this macro.
